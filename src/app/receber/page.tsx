...
export default function ContasAReceberPage() {
  ...
  async function handleSave(dados: Omit<Lancamento, "id" | "criadoEm">, mesesRecorrencia?: number) {
    try {
      if (editando) {
        ...
        setToast({ message: "Lançamento cadastrado com sucesso!", type: "success" });
      }
  ...
  async function handleMarcarPago(id: string) {
    try {
      ...
      setToast({ message: "Marcado como recebido!", type: "success" });
      carregar();
    } catch {
  ...
  const recebidos = doMesAtual.filter((l) => l.status === "pago");
  ...
  return (
    <div className="flex min-h-screen bg-slate-950">
        ...
        <div className="max-w-6xl mx-auto space-y-4 md:space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
            {[
              ...
              { label: "Recebidos", value: formatCurrency(recebidos.reduce((s, l) => s + l.valor, 0)), cls: "text-emerald-400" },
      ...
      <Modal
        ...
        title={editando ? "Editar lançamento" : "Nova conta a receber"}
      >
        <LancamentoForm
          tipo="receber"
